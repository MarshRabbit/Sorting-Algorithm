...
![그림1](https://user-images.githubusercontent.com/80511335/117337233-19907a80-aed8-11eb-961d-682919912827.png)

시간복잡도를 각 정렬끼리 비교해 보면 시간복잡도가 같은 O(N<sup>2</sup>)지만 연산 시간에 차이가 남을 볼 수 있다.

버블 정렬은 이미 정렬된 경우라면 한번만 돌면 되기 때문에 최선의 성능을 보여줄 수 있지만 거의 모든 상황에서 가장 최악의 성능을 보여준다.

선택 정렬은 비교햐는 횟수에 비해 교환이 적게 일어나므로 버블정렬보단 확실히 빠른것을 알 수 있다.

삽입 정렬은 최악일 경우가 O(N<sup>2</sup>)이고 최선의 경우 O(N)인 시간복잡도를 가지고 있어 엄청나게 빠른 효율성을 가지고 있다. 이론상 버블 정렬이나 선택 정렬과 같은 시간복잡도를 갖음에도 평균 비교 횟수에 대한 기대값이 상대적으로 적기 때문에 평균 시간복잡도가 O(N<sup>2</sup>) 인 정렬 알고리즘 중에서는 빠른편에 속한다.

쉘 정렬은 이론상 삽입 정렬보다 더욱 성능이 좋아야 하지만 간격의 설정을 잘못한 것인지 비교 테스트를 진행할 때에는 삽입 정렬보다 느린 속도를 보여주었다. 쉘 정렬은 간격을 어떻게 설정하느냐에 따라서 성능이 천차만별이기 때문에 간격 설정에 유의가 필요하다
